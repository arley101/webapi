# Archivo para el servicio 'linkedin_ads' generado automáticamente
...
def linkedin_get_ad_accounts(
    params: Optional[Dict[str, Any]] = Body(None),
    client: AuthenticatedHttpClient = Depends(get_authenticated_http_client)
):
    """Ruta autogenerada para linkedin_get_ad_accounts"""
    final_params = params or {}
    result = linkedin_ads_actions.linkedin_get_ad_accounts(client=client, params=final_params)
...
def linkedin_list_campaigns(
    params: Optional[Dict[str, Any]] = Body(None),
    client: AuthenticatedHttpClient = Depends(get_authenticated_http_client)
):
    """Ruta autogenerada para linkedin_list_campaigns"""
    final_params = params or {}
    result = linkedin_ads_actions.linkedin_list_campaigns(client=client, params=final_params)
    if isinstance(result, dict) and result.get('status') == 'error':
        raise HTTPException(status_code=result.get('http_status', 500), detail=result)
    if isinstance(result, Response):
        return result
...
def linkedin_get_basic_report(
    params: Optional[Dict[str, Any]] = Body(None),
    client: AuthenticatedHttpClient = Depends(get_authenticated_http_client)
):
    """Ruta autogenerada para linkedin_get_basic_report"""
    final_params = params or {}
    result = linkedin_ads_actions.linkedin_get_basic_report(client=client, params=final_params)
    if isinstance(result, dict) and result.get('status') == 'error':
        raise HTTPException(status_code=result.get('http_status', 500), detail=result)
    if isinstance(result, Response):
        return result
