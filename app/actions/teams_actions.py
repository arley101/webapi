...
def list_joined_teams(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if raw_max_items is not None:
        try:
            ...
            logger.warning(f"{action_name}: 'max_items_total' ('{raw_max_items}') no es un entero válido.")
...
def list_channels(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    channel_read_scope = getattr(settings, 'GRAPH_SCOPE_CHANNEL_READ_ALL', getattr(settings, 'GRAPH_SCOPE_TEAMS_READ_BASIC_ALL', settings.GRAPH_API_DEFAULT_SCOPE))
...
def list_channel_messages(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    max_items_total: Optional[int] = None
    if raw_max_items is not None:
        try: max_items_total = int(raw_max_items)
        except (ValueError, TypeError): logger.warning(f"{action_name}: 'max_items_total' ('{raw_max_items}') no es un entero válido.")
    query_api_params: Dict[str, Any] = {'$top': top_per_page}
    ...
    channel_read_scope = getattr(settings, 'GRAPH_SCOPE_CHANNEL_MESSAGE_READ_ALL', getattr(settings, 'GRAPH_SCOPE_CHANNEL_READ_ALL', settings.GRAPH_API_DEFAULT_SCOPE))
...
def list_chats(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    chat_read_scope = getattr(settings, 'GRAPH_SCOPE_CHAT_READ_ALL', getattr(settings, 'GRAPH_SCOPE_CHAT_READBASIC_ALL', settings.GRAPH_API_DEFAULT_SCOPE))
...
def create_chat(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if chat_type == "oneonone" and len(members_payload) != 2: return _handle_teams_api_error(ValueError("Para chat 'oneOnOne', 'members' debe tener 2 miembros."), action_name, params)
    if chat_type == "group" and not topic: return _handle_teams_api_error(ValueError("'topic' es requerido para chats grupales."), action_name, params)
...
def send_chat_message(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    message_content: Optional[str] = params.get("content")
    content_type: str = params.get("content_type", "HTML").upper()
    if not chat_id or message_content is None: return _handle_teams_api_error(ValueError("'chat_id' y 'content' son requeridos."), action_name, params)
...
def schedule_meeting(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not user_identifier_organizer: return {"status": "error", "action": action_name, "message": "'organizer_user_id' es requerido.", "http_status": 400}
    ...
    if not subject or not start_datetime_str or not end_datetime_str: return _handle_teams_api_error(ValueError("'subject', 'start_datetime', 'end_datetime' requeridos."), action_name, params)
    try:
        datetime.fromisoformat(start_datetime_str.replace('Z', '+00:00'))
        datetime.fromisoformat(end_datetime_str.replace('Z', '+00:00'))
    except ValueError as ve: return _handle_teams_api_error(ValueError(f"Formato de fecha inválido: {ve}."), action_name, params)
    url = f"{settings.GRAPH_API_BASE_URL}/users/{user_identifier_organizer}/events"
    payload = { "subject": subject, "start": {"dateTime": start_datetime_str, "timeZone": params.get("timezone", "UTC")}, "end": {"dateTime": end_datetime_str, "timeZone": params.get("timezone", "UTC")}, "isOnlineMeeting": True, "onlineMeetingProvider": "teamsForBusiness" }
...
def get_meeting_details(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not user_identifier_owner: return {"status": "error", "action": action_name, "message": "'owner_user_id' es requerido.", "http_status": 400}
    ...
    try:
        ...
        # CORRECCIÓN: 'event_data' ya es un dict
...
def list_members(client: AuthenticatedHttpClient, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if team_id:
        ...
        scope_to_use = getattr(settings, 'GRAPH_SCOPE_TEAM_MEMBER_READ_ALL', getattr(settings, 'GRAPH_SCOPE_GROUP_READ_ALL', settings.GRAPH_API_DEFAULT_SCOPE))
    else:
        url_base = f"{settings.GRAPH_API_BASE_URL}/chats/{chat_id}/members"
        scope_to_use = getattr(settings, 'GRAPH_SCOPE_CHAT_MEMBER_READ_ALL', getattr(settings, 'GRAPH_SCOPE_CHAT_READBASIC_ALL', settings.GRAPH_API_DEFAULT_SCOPE))
    top_per_page: int = min(int(params.get('top_per_page', 25)), getattr(settings, 'DEFAULT_PAGING_SIZE', 50))
    raw_max_items = params.get('max_items_total')
    max_items_total: Optional[int] = None
    if raw_max_items is not None:
